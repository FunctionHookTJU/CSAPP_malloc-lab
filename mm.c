...
void *mm_malloc(size_t size){
    ...
    if (size <= DSIZE){
        ...
        size = ALIGN(size + DSIZE);//DSIZE是头部和脚部的大小，size是有效载荷，加上才是一整个块的大小
    }
    int listnumber = 0;
...
static void insert_node(void *ptr, size_t size)/*该函数将一个空闲块插入到分离空闲链表中，首先根据块的大小找到对应的链表，然后在该链表中寻找合适的插入位置，保证链表中块按大小从小到大排序，最后根据不同情况完成插入操作。*/
{
